...
describe('GitHub PR Analyzer', () => {
  ...
  test('demonstrate beautified output formatting', async () => {
    const owner = 'WillBooster';
    const repo = 'gen-pr';
    const startDate = '2025-06-01';
    const endDate = '2025-06-10';
    ...
    // Import the function dynamically to avoid import issues
    const { analyzePullRequestsByDateRange, formatDateRangeAnalysisResult, formatAnalysisResult } = await import(
      '../../src/analyzer.js'
    );

    // Test with AI email specified to show the full beautified output
    const result = await analyzePullRequestsByDateRange(owner, repo, startDate, endDate, {
      aiEmails: ['bot@willbooster.com'],
    });

    const formattedOutput = formatDateRangeAnalysisResult(result, {
      aiEmails: ['bot@willbooster.com'],
    });

    console.log('\n=== BEAUTIFIED DATE RANGE OUTPUT ===');
    console.log(formattedOutput);
    console.log('=== END BEAUTIFIED OUTPUT ===\n');

    // Also test single PR formatting
    const prResult = await analyzePullRequest(owner, repo, 65, {
      aiEmails: ['bot@willbooster.com'],
    });

    const formattedPROutput = formatAnalysisResult(prResult, {
      aiEmails: ['bot@willbooster.com'],
    });

    console.log('\n=== BEAUTIFIED SINGLE PR OUTPUT ===');
    console.log(formattedPROutput);
    console.log('=== END BEAUTIFIED PR OUTPUT ===\n');

    // Basic assertions to ensure the formatting works
    expect(formattedOutput).toContain('CONTRIBUTION ANALYSIS REPORT');
    expect(formattedOutput).toContain('HUMAN vs AI BREAKDOWN');
    expect(formattedOutput).toContain('INDIVIDUAL CONTRIBUTIONS');
    expect(formattedOutput).toContain('📊 SUMMARY STATISTICS');
    expect(formattedOutput).toContain('👥 Human Contributors');
    expect(formattedOutput).toContain('🤖 AI Contributors');

    expect(formattedPROutput).toContain('PR #65   ANALYSIS REPORT');
    expect(formattedPROutput).toContain('HUMAN vs AI BREAKDOWN');
    expect(formattedPROutput).toContain('INDIVIDUAL CONTRIBUTIONS');
  }, 120000);
});
